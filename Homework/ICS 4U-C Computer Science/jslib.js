function bubble_sorting_function(usr_array){
	var i, j;
...
function revs_bubble_sorting_function(usr_array){
	var i, j;
	for (i = 0; i < usr_array.length; ++i){
		for (j = 0; j < usr_array.length - i - 1; ++j){
			if (usr_array[j] < usr_array[j + 1]){
...
function check_nonNum(usr_array){
	var counter;
	for (counter = 0; counter < usr_array.length; ++counter){
		if (isNaN(usr_array[counter]) == true){
			var choice = confirm("The element you have just entered is not a number !!!\n"
